...
const kakaoCallback = async (req, res) => {
    ...
    if (!code) {
        return res.redirect("https://localhost:3000/login/fail");
    }
    ...
    try {
        ...
        // JWT만 프론트로 전달
        res.redirect(`https://localhost:3000/login/success?token=${jwtToken}`);
    } catch (err) {
        console.error(err);
        res.redirect("https://localhost:3000/login/fail");
    }
};
